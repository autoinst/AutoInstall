...
func DownloadServerJar(version, loader, librariesDir string, downloadSource string, maxRetries int) error {
	...
	case "forge":
		...
		} else if compareMCVersion(version, "1.16.5") <= 0 {
	...
	} else if loader == "forge" && compareMCVersion(version, "1.16.5") <= 0 {
		serverPath = filepath.Join(".", serverFileName)
	} else {
